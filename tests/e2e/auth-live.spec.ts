...
test.describe('Live Authentication Flow', () => {
  ...
  test('should show child sign-in modal and validate form', async ({
    page,
  }) => {
    await page.goto('/');
  ...
  test('should navigate to parent sign-in', async ({ page }) => {
    ...
    await page
      .locator('header')
      .getByRole('button', { name: 'PARENT LOGIN' })
      .click();
  ...
  test('should complete onboarding success flow', async ({ page }) => {
    ...
    await expect(
      page.getByText("Your child's Onda account is ready!")
    ).toBeVisible();

    // Should have navigation buttons
    await expect(
      page.getByRole('button', { name: 'GO TO PARENT DASHBOARD' })
    ).toBeVisible();
    await expect(
