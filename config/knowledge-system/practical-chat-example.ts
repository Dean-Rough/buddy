...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      response:
        'whoops something went weird there - wanna try saying that again?',
      error: true,
    });
  }
}
...
function generateTypingPattern(response: string, age: number): any {
  ...
  words.forEach((word, i) => {
    const wordTime = (word.length / 5) * (60 / baseWPM) * 1000;
...
function getEmergencyResponse(age: number, category: string): string {
  const responses = {
    self_harm: {
      ...
      '9-10':
        "i'm really worried about what you're saying. this is too big for us to handle alone - please talk to a trusted adult right away",
      '11-12':
        "what you're going through sounds incredibly hard and i'm genuinely worried. please reach out to a trusted adult immediately - you deserve support",
    },
    abuse: {
      '7-8':
        "that doesn't sound right at all. you should tell a grown-up you trust about this - like a teacher or family member who makes you feel safe",
      '9-10':
        "what you're describing isn't okay and it's not your fault. please tell a trusted adult - a teacher, counselor, or family member who can help",
      '11-12':
        "this is serious and you don't have to deal with it alone. please talk to a trusted adult immediately - what's happening to you isn't okay",
    },
    default: {
      '7-8':
        "this seems really important. let's get a grown-up to help with this, okay?",
      '9-10':
        'this is something a trusted adult would be much better at helping with. can you talk to someone?',
      '11-12':
        'this is beyond what i can help with - please reach out to a trusted adult who can properly support you',
    },
  };
